...
const Patient_create = () => {
  ...
  const [notification, setNotification] = useState(null); // To handle success/error messages
  ...
  const handleCreatePatient = async () => {
    const result = await dispatch(createPatient({ data, token }));
      ...
      <div style={{ display: "block", width: "100%", height: "fit-content", padding: "10px" }}>
        <div className=' md:w-3/5 sm:w-full'>
          ...
          <div className='md:w-10/12 m-2 sm:w-full'>
            <label className={` text-3xl font-medium block leading-normal m-2 break-words ${mode === 'dark' ? 'text-blue-100' : 'text-blue-800'}`}> Gender</label>
            <div className={`inline-block m-2 w-fit h-fit p-2 rounded ${gender === 'true' ? 'bg-green-500' : 'bg-blue-200'}`} onClick={() => handleRadioChange('true')}>Male</div>
            <div className={`inline-block m-2 w-fit h-fit p-2 rounded ${gender === 'false' ? 'bg-green-500' : 'bg-blue-200'} `} onClick={() => handleRadioChange('false')}>Female</div>
          </div>
          <div className='md:w-10/12 m-2 flex flex-col sm:w-full'>
          ...
          <div className='md:w-10/12 m-2 flex flex-col sm:w-full'>
            ...
            <input className='form-input ' type='text' placeholder='patient Height' value={height} onChange={e => setHeight(e.target.value)} />
          </div>
          <div className='md:w-10/12 m-2 flex flex-col sm:w-full'>
            <label className={` text-3xl font-medium leading-normal m-2 break-words ${mode === 'dark' ? 'text-blue-100' : 'text-blue-800'}`}>weight</label>
            <input className='form-input ' type='text' placeholder='patient weight ' value={weight} onChange={e => setWeight(e.target.value)} />
          </div>
          <button className='create-btn bg-green-500' onClick={handleCreatePatient}>Create</button>
        </div>
      </div>
    </div>
  )
